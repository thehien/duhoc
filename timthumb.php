...
class timthumb
{
    ...
    public function __construct()
    {
        ...
        if (BLOCK_EXTERNAL_LEECHERS && array_key_exists('HTTP_REFERER',
    ...
    protected function processImageAndWriteToCache($localImage)
    {
        ...
        if ($zoom_crop > 0) {
            ...
            imagecopyresampled($canvas, $image, $origin_x, $origin_y, $src_x, $src_y, $new_width, $new_height, $src_w,
                $src_h);
                ...
                if (isset ($imageFilters[$filterSettings[0]])) {
                    ...
                    switch ($imageFilters[$filterSettings[0]][1]) {
                        ...
                        case 3:
                            ...
                            imagefilter($canvas, $imageFilters[$filterSettings[0]][0], $filterSettings[1],
                                $filterSettings[2], $filterSettings[3]);
                            break;

                        case 4:

                            imagefilter($canvas, $imageFilters[$filterSettings[0]][0], $filterSettings[1],
                                $filterSettings[2], $filterSettings[3], $filterSettings[4]);
                            break;
        ...
        file_put_contents($tempfile4,
            $this->filePrependSecurityBlock . $imgType . ' ?' . '>'); //6 extra bytes, first 3 being image type
    ...
    protected function calcDocRoot()
    {
        ...
        if (!isset($docRoot)) {
            ...
            if (isset($_SERVER['PATH_TRANSLATED'])) {
                $docRoot = str_replace('\\', '/', substr(str_replace('\\\\', '\\', $_SERVER['PATH_TRANSLATED']), 0,
    ...
    protected function getLocalImagePath($src)
    {
        ...
        if (!$this->docRoot) {
            ...
            $file = preg_replace('/^.*?([^\/\\\\]+)$/', '$1',
                $src); //strip off any path info and just leave the filename.
            if (is_file($file)) {
    ...
    protected function serveWebshot()
    {
        ...
        $url = preg_replace('/[^A-Za-z0-9\-\.\_:\/\?\&\+\;\=]+/', '',
            $url); //RFC 3986 plus ()$ chars to prevent exploit below. Plus the following are also removed: @*!~#[]',
    ...
    protected function setMemoryLimit()
    {
        ...
        if ($inibytes < $ourbytes) {
            ...
            $this->debug(3,
                "Not adjusting memory size because the current setting is " . $inimem . " and our size of " . MEMORY_LIMIT . " is smaller.");
        }
    }
