...
function news_process()
{
	...
	switch($main)
	{
		...
		if($action_views == ''){return $function->msg_box_status(msg_box_action,$msg_time,URL_ADMIN."?module=homes",2);}
			...
			$url_limit = ''; $url_order = ''; $url_category = ''; $url_page = ''; $base_url = '';
			if($page){$smarty->assign("url_page","&page=$page"); $url_page ="&page=$page";}
		...
		if($action_insert == '' and $action_edit == ''){return $function->msg_box_status(msg_box_action,$msg_time,URL_ADMIN."?module=homes",2);}
			if($submit)
			{
				...
				if($_POST['news_url']){$data['info']['news_url']=$function->character_url(isset($_POST['news_url'])?$_POST['news_url']:"");}
				else{$data['info']['news_url'] = $function->character_url(isset($_POST['news_name']) ? $_POST['news_name'] : "");}
				...
				else{$data['info']['seo_title'] = $function->sql_injection(isset($_POST['news_name']) ? $_POST['news_name'] : "");}
				if($_POST['seo_key']){$data['info']['seo_key'] = $function->sql_injection(isset($_POST['seo_key']) ? $_POST['seo_key'] : "");}
				else{$data['info']['seo_key'] = $function->sql_injection(isset($_POST['news_name']) ? $_POST['news_name'] : "");}
				if($_POST['seo_desc']){$data['info']['seo_desc'] = $function->FixQuotes(isset($_POST['seo_desc']) ? $_POST['seo_desc'] : "");}
				else{$data['info']['seo_desc'] = $function->FixQuotes(isset($_POST['news_content']) ? $_POST['news_content'] : "");}
				...
				{	
					if($id_edit){
						...
						else{$data['info']['news_img']=$image_old;}
