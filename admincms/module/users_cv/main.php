...
function users_cv_process()
{
	...
	switch($main)
	{
		...
		$_SESSION[$module]['url_views'] =$function->FixQuotes($_SERVER['REQUEST_URI']);
			...
			$url_limit = ''; $url_order = ''; $url_category = ''; $url_page ='';
		...
		if($action_insert == '' and $action_edit == ''){return $function->msg_box_status(msg_box_action,$msg_time,URL_ADMIN."?module=homes",2);}
			if($submit)
			{
				...
				{	
					...
					{
						if($checks_user >=1){return $function->msg_box_status(msg_box_erro_user,$msg_time,$_SESSION[$module]['url_views'],2);}
						elseif($checks_email >=1){return $function->msg_box_status(msg_box_erro_email,$msg_time,$_SESSION[$module]['url_views'],2);}
						else
						{
		...
		if($action_edit == ''){return $function->msg_box_status(msg_box_action,$msg_time,URL_ADMIN."?module=homes",2);}
			...
			if($check != null){
				foreach($check as $id){			
					...
					$id = $function->sql_injection(intval($id));	
					$Process->update_action_user($id,$action_views,$action_insert,$action_edit,$action_delete,$status);		
				}
			}
			return $function->msg_box_status(msg_box_edit_succ,$msg_time,$_SESSION[$module]['url_views'],1);
		break;
