...
//Views tree
    function views_tree_arrays(
        ...
        if ($name_search == '') {
            ...
            $sr_search = "and name like '%" . $name_search . "%'";
        }
        ...
        if ($db->db_numrows($res) > 0) {
            ...
            foreach ($rows as $tree) {
                ...
                if ($level == 0) {
                    $str = $spacing . '<i style="color:#337ab7" class="fa ' . $tree['icon'] . '"></i>&nbsp;<b>' . $tree['name'] . '</b>';
                ...
                $tree_arrays = $this->views_tree_arrays($name_search = '', $order = '', $sc = '', $tree['module_id'],
...
//select tree
    function select_tree_arrays($check_id, $parent = 0, $spacing = '', $tree_arrays = '', $level = 0)
    {
        ...
        $str = "";
        if ($check_id > 0) {
            $str = "and a.parent <> $check_id and a.module_id <> $check_id";
        }
        ...
        if ($db->db_numrows($res) > 0) {
            ...
            foreach ($rows as $tree) {
                if ($level == 0) {
                    ...
                    $str = $spacing . '&raquo;&nbsp;' . $tree['name'];
                }

                if ($level == 0) {
                    $str_box = $spacing . '<b>' . $tree['name'] . '</b>';
                } else {
                    $str_box = $spacing . $tree['name'];
                }
                ...
                $level++;
                $db->db_freeresult($res);
                $tree_arrays = $this->select_tree_arrays($check_id, $tree['module_id'],
                    $spacing . '&nbsp;&nbsp;&nbsp;&nbsp;', $tree_arrays, $level);
                $level--;
            }
        }
