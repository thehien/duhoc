	...
	switch($main)
	{
		...
		if($action_insert == '' and $action_edit == ''){return $function->msg_box_status(msg_box_action,$msg_time,URL_ADMIN."?module=homes",2);}
			if($submit)
			{
				...
				if($_POST['category_url']){$data['info']['category_url']=$function->character_url(isset($_POST['category_url'])?$_POST['category_url']:"");}
				else{$data['info']['category_url'] = $function->character_url(isset($_POST['category_name']) ? $_POST['category_name'] : "");}
				...
				if($_POST['seo_title']){$data['info']['seo_title'] = $function->sql_injection(isset($_POST['seo_title']) ? $_POST['seo_title'] : "");}
				else{$data['info']['seo_title'] = $function->sql_injection(isset($_POST['category_name']) ? $_POST['category_name'] : "");}
				
				if($_POST['seo_key']){$data['info']['seo_key'] = $function->sql_injection(isset($_POST['seo_key']) ? $_POST['seo_key'] : "");}
				else{$data['info']['seo_key'] = $function->sql_injection(isset($_POST['category_name']) ? $_POST['category_name'] : "");}
				
				if($_POST['seo_desc']){$data['info']['seo_desc'] = $function->FixQuotes(isset($_POST['seo_desc']) ? $_POST['seo_desc'] : "");}
				else{$data['info']['seo_desc'] = $function->FixQuotes(isset($_POST['category_content']) ? $_POST['category_content'] : "");}
				...
				{	
					if($id_edit)
					{
						...
						{
							unlink(IMG_UPLOAD.$url.'/'.$image_old); $data['info']['category_img']="";
						}
						else{$data['info']['category_img']=$image_old;}
		...
		if($action_delete == ''){return $function->msg_box_status(msg_box_action,$msg_time,URL_ADMIN."?module=homes",2);}
			...
			if($check != null)
			{
				...
				{										
					...
					elseif ($checks_product > 0){return $function->msg_box_status(msg_box_erro_parent,$msg_time,$_SESSION[$module]['url_views'],2);}
					elseif ($checks_news > 0){return $function->msg_box_status(msg_box_erro_parent,$msg_time,$_SESSION[$module]['url_views'],2);}
					else
					{										
