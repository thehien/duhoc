...
//Views tree
function views_tree_arrays($name_search='',$order='',$sc='',$parent_id=0,$spacing='',$tree_arrays='',$level=0)
{
	global $db;
	...
	else{$sr_search="and category_name like '%".$name_search."%'";}
	...
	if($db->db_numrows($res) > 0)
	{
		...
		foreach($rows as $tree)
		{
			...
			if($level == 0){$str = $spacing.'&raquo;&nbsp;<b>'.$tree['category_name'].'</b>';}
			elseif($level == 1){$str = $spacing.'&nbsp;&nbsp;&nbsp;&nbsp;&raquo;&raquo;&nbsp;<b>'.$tree['category_name'].'</b>';}
			elseif($level == 2){$str = $spacing.'&nbsp;&nbsp;&nbsp;&raquo;&raquo;&raquo;&nbsp;<b>'.$tree['category_name'].'</b>';}
			$i='';
			$tree_arrays[] = array
			(
				"category_name" => $str,
			...
			$tree_arrays = $this->views_tree_arrays($name_search='',$order='',$sc='',$tree['category_id'],$spacing.'',$tree_arrays,$level);
			$level--;
		}
	}return $tree_arrays;
}
...
function select_tree_arrays($check_id,$news_url='',$parent_id=0,$spacing='',$tree_arrays='',$level=0)
{
	...
	if(!is_array($tree_arrays))$tree_arrays = array();
	$str = "";
	if($news_url == 1){$str.="and a.news_url=''";}
	if($news_url == 2){$str.="and a.news_url='menu/'";}
	if($check_id > 0)$str.= "and a.parent_id <> $check_id and a.category_id <> $check_id";
	...
	if($db->db_numrows($res) > 0)
	{
		...
		{
			if($level == 0){$str = $spacing.'&raquo;&nbsp;<b>'.$tree['category_name'].'</b>';}
			else{$str = $spacing.'&raquo;&raquo;&nbsp;'.$tree['category_name'];}
			...
			$level++;
			$db->db_freeresult($res);
			$tree_arrays = $this->select_tree_arrays($check_id,$news_url='',$tree['category_id'],$spacing.'&nbsp;&nbsp;&nbsp;&nbsp;',$tree_arrays,$level);
			$level--;
		}
	}return $tree_arrays;
}
